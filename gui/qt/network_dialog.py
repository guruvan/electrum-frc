...
class NetworkDialog(QDialog):
    def __init__(self, network, config, parent):
        ...
        hbox.addStretch(50)
        msg = _("Electrum-FRC sends your wallet addresses to a single server, in order to receive your transaction history.") + "\n\n" \
            + _("In addition, Electrum-FRC connects to several nodes in order to download block headers and find out the longest blockchain.") + " " \
        ...
        msg = _("If auto-connect is enabled, Electrum-FRC will always use a server that is on the longest blockchain.") + " " \
            + _("If it is disabled, Electrum-FRC will warn you if your server is lagging.")
